...
export function compile<P extends ParamData = ParamData> (
  ...
  const { encode = encodeURIComponent, delimiter = DEFAULT_DELIMITER } =
    options
  const data = path instanceof TokenData ? path : parse(path as string, options)
 ...
 */
export type Path = string | RegExp | TokenData
...
export function pathToRegexp (
  ...
  for (const input of pathsToArray(path, [])) {
    if (input instanceof RegExp) {
      // For RegExp inputs, use the pattern directly
      sources.push(input.source)
    } else {
      const data = input instanceof TokenData ? input : parse(input as string, options)
